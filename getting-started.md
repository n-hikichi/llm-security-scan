# LLM Security Scan へようこそ
## 初心者向けガイド

---

# 目次

## メインコンテンツ
- LLM Security Scan とは
- 従来のセキュリティスキャンとの違い
- 必要な準備
- セットアップ手順（5分でできる）
- 実行フロー
- 期待される成果物

## 付録
- 付録 A：v1.0.0 仕様
- 付録 B：よくある質問（Q&A）
- 付録 C：Python 拡張概要

---

# LLM Security Scan とは

**LLM Security Scan** = Claude AI を活用したセキュリティスキャンツール

## 特徴
- **プロジェクト型の自動判断** - リポジトリ構造から JavaScript/Vue.js/Express.js を自動認識
- **生成AI駆動の分析** - ルールベースではなく、Claude の言語理解能力で文脈を考慮した検出
- **MCP Serena との連携** - コードベースのディープな分析が可能
- **入出力が明確** - 入力：リポジトリ、出力：詳細レポート（ターミナル表示 + ファイル保存可能）

---

# 従来のセキュリティスキャンとの違い

| 項目 | 従来のツール | LLM Security Scan |
|------|------------|------------------|
| 分析方式 | ルールベース | AI ベース（Claude） |
| 検出能力 | パターンマッチング | 文脈を理解した分析 |
| 誤検知 | 多い | 少ない |
| カスタマイズ | 難しい | Claude で調整可能 |
| 学習時間 | 短い | 中程度（初期セットアップ） |

---

# 必要な準備

## 前提条件
- Claude Code CLI がインストール済み
- 対象プロジェクトが git clone 済み
- uv（Python パッケージマネージャ）がインストール可能

## 所要時間
- セットアップ：**約 15-20 分**
- 初回スキャン：**約 10 分**（プロジェクト規模による）
- 2 回目以降：**約 5-10 分**

---

# セットアップ手順（ステップ 1/3）

## ステップ 1: uv をインストール

```bash
curl -LsSf https://astral.sh/uv/install.sh | sh
```

※ 一度だけ実行すればOK

---

# セットアップ手順（ステップ 2/3）

## ステップ 2: MCP Serena をセットアップ

スキャン対象ディレクトリで以下を実行：

```bash
claude mcp add serena -- uvx --from git+https://github.com/oraios/serena \
  serena-mcp-server --context ide-assistant --project $(pwd) \
  --enable-webdashboard=false
```

その後、Claude CLI を再起動

---

# セットアップ手順（ステップ 3/3）

## ステップ 3: Skill をインストール

1. GitHub Releases から `llm-security-scan.zip` をダウンロード
   ```bash
   curl -L https://github.com/n-hikichi/llm-security-scan/releases/latest/download/llm-security-scan.zip \
     -o llm-security-scan.zip
   ```

2. 展開する
   ```bash
   unzip llm-security-scan.zip
   ```

3. Claude Code に skill が認識されたか確認
   ```
   > skill
   ```

---

# 実行フロー

## スキャン開始から結果取得まで

```
1. Claude CLI 起動（プロジェクトディレクトリで）
   ↓
2. "セキュリティスキャンを実行して" と指示
   ↓
3. MCP Serena が初期化（初回のみ）
   ↓
4. Claude AI がコードを分析
   ↓
5. セキュリティレポート生成
   ↓
6. 結果をファイルに保存（オプション）
```

⏱️ **所要時間：約 10 分**

---

# スキャンで検出される項目

## セキュリティ脆弱性
- ✅ XSS（クロスサイトスクリプティング）
- ✅ SQL インジェクション
- ✅ 認証・認可の問題
- ✅ 依存関係の脆弱性
- ✅ 暗号化関連の問題

## コード品質
- ✅ セキュリティアンチパターン
- ✅ 危険な関数の使用
- ✅ 設定ファイルの問題

---

# 入力と出力の流れ

## 入力（何をスキャンするか）

```
Git リポジトリ（ローカルで展開済み）
  ↓
LLM Security Scan が自動判断
  ├─ package.json で JavaScript/Node.js を検出
  ├─ Vue.js/React/Express.js のプレームワークを判断
  └─ プロジェクト構造から対応言語を認識
```

**自動判断：** リポジトリから言語・フレームワークを自動判定

---

# 出力と結果の取得方法

## 出力（スキャン結果をどこで見るか）

| 出力先 | 用途 | 操作 |
|------|------|------|
| **ターミナル（Claude CLI）** | リアルタイム確認 | スキャン実行中に自動表示 |
| **ファイル保存** | 記録・共有・監視 | `> レポートをファイルに保存して` で Markdown ファイル生成 |

## 例：出力ファイル
```
security-report-2026-03-10.md
```

---

# 期待される成果物

## スキャン完了後、以下を得られます

### セキュリティレポート
```
- 検出された脆弱性リスト
- 重大度別の分類（Critical / High / Medium / Low）
- 具体的な修正方法の提案
- 影響範囲の説明
```

### ファイルに保存
```bash
> 得られたレポートをファイルに保存して。
```

---

# 付録 A：v1.0.0 仕様

## スコープ：JavaScript/Node.js プロジェクト対応

### プロジェクト型の自動判定
- **判定基準：** `package.json` の存在とディレクトリ構造
- **対応フレームワーク：** Vue.js、React、Express.js、NestJS など
- **プロジェクト分類：**
  - **CLI（クライアント）** - Vue.js/React 等のフロントエンド
  - **SV（サーバー）** - Express.js/NestJS 等のバックエンド
  - **Hybrid（ハイブリッド）** - フルスタックフレームワーク

### セキュリティチェックリスト

#### クライアントサイド（CLI）向け
- ✅ 認証・認可：トークン管理、ストレージセキュリティ、自動ログアウト
- ✅ XSS 対策：入力エスケープ、v-html の使用、DOM 操作
- ✅ API 通信：HTTPS 強制、認証ヘッダー、エラーハンドリング
- ✅ 依存関係：既知の脆弱性、パッケージバージョン
- ✅ データ保護：localStorage/sessionStorage の使用、機密情報露出

#### サーバーサイド（SV）向け
- ✅ 認証・認可：JWT 管理、パスワードハッシュ、セッション管理
- ✅ データベース：SQL インジェクション、接続セキュリティ、暗号化
- ✅ 入力検証：サニタイゼーション、型チェック、ホワイトリスト検証
- ✅ CORS とヘッダー：セキュリティヘッダー、CORS 設定
- ✅ エラーハンドリング：情報漏洩、スタックトレース、ログ管理
- ✅ レート制限：DoS 対策、API 制限

---

# 対応言語とプロジェクト型

## v1.0.0 対応言語

| 言語 | フレームワーク | サポート | 備考 |
|------|-------------|---------|------|
| **JavaScript** | Vue.js, React, Angular | ✅ 完全対応 | クライアント向けチェック |
| **TypeScript** | Next.js, Nuxt.js, Remix | ✅ 完全対応 | 型安全性チェック含む |
| **Node.js** | Express.js, NestJS, Koa, Fastify | ✅ 完全対応 | サーバー向けチェック |

## 将来の拡張可能性

### 技術的に可能な言語
Claude AI は任意のプログラミング言語を解析可能なため、以下の言語への拡張は技術的に実現可能です：

| 言語 | 実装の要件 | 優先度 |
|------|---------|-------|
| **Python** | `requirements.txt`/`pyproject.toml` 検出 + Django/Flask チェックリスト | 🔵 中 |
| **Java** | `pom.xml`/`build.gradle` 検出 + Spring/Jakarta チェックリスト | 🔵 中 |
| **Go** | `go.mod` 検出 + Go 固有チェックリスト | 🟡 低 |
| **その他言語** | 言語別検出スクリプト + セキュリティプロンプト | 🟡 低 |

**注：** 対応希望がある場合は GitHub Issues で報告してください

---

# 付録 B：よくある質問（Q&A）

### Q1: スキャン中、npm audit の実行を聞かれた場合？
**A:** 別フェーズで実行するため「実行しない」を選択

### Q2: スキャン結果の信頼性は？
**A:** Claude AI による分析なので、最終確認はチーム内で行うことをお勧め

### Q3: 同じプロジェクトを再スキャンできる？
**A:** はい、何回でも実行可能（2回目以降は高速化）

### Q4: スキャン結果をテキストファイルに保存できる？
**A:** はい、スキャン完了後に「レポートをファイルに保存して」と指示

### Q5: Python/Java プロジェクトもスキャンできる？
**A:** 現在は JavaScript/Node.js が公式対応。他言語は技術的に検討中

### Q6: 出力結果はどこで見られる？
**A:** 2つの方法
  - リアルタイム：Claude CLI のターミナル画面
  - 保存：Markdown ファイルとして生成・保存可能

---

# 付録 C：Python 拡張概要

## Python 対応への技術的アプローチ

LLM Security Scan は現在 JavaScript/Node.js に特化していますが、技術的には Python への拡張は実現可能です。

### 実装に必要な修正

#### 1. プロジェクト検出スクリプト（detect_project_type.py）の拡張

```python
# 現在：package.json で JavaScript を検出
# 拡張：以下を追加

def detect_language():
    if os.path.exists('package.json'):
        return 'javascript'
    elif os.path.exists('requirements.txt') or os.path.exists('pyproject.toml'):
        return 'python'
    elif os.path.exists('pom.xml') or os.path.exists('build.gradle'):
        return 'java'
```

#### 2. Python セキュリティプロンプト（references/python-security-prompt.md）の新規作成

**検査項目：**
- SQL インジェクション（SQLAlchemy、raw SQL）
- 認証・認可（Flask-Login、Django auth、JWT）
- CSRF 対策（Flask-WTF、Django CSRF トークン）
- パスワード管理（bcrypt、werkzeug.security）
- 依存関係の脆弱性（pip、Poetry）
- シークレット管理（python-dotenv、環境変数）

#### 3. フレームワーク別チェックリスト

| フレームワーク | 検査項目 | 優先度 |
|-------------|---------|--------|
| **Django** | ORM セキュリティ、ミドルウェア、権限管理 | 🔴 高 |
| **Flask** | 入力検証、セッション管理、エラーハンドリング | 🔴 高 |
| **FastAPI** | 非同期処理の脆弱性、依存性注入 | 🟡 中 |
| **SQLAlchemy** | プリペアドステートメント、SQL インジェクション | 🔴 高 |

### 実装手順

1. **環境検出機能追加** - 2-3 時間
2. **セキュリティプロンプト作成** - 4-6 時間
3. **テストとドキュメント** - 2-3 時間

**総実装時間：** 約 8-12 時間

### 技術的な検討事項

- Claude AI は Python コードを完全に理解できるため、JavaScript と同等の分析精度が期待できる
- MCP Serena も Python プロジェクトに対応可能
- 既存の JavaScript チェックロジックの多くを Python に適用可能
