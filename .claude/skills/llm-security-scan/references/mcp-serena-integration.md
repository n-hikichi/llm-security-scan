...
> **前提**: Skill 実行時には、スキャン対象リポジトリ (SUT) のディレクトリで `claude` を起動している前提です。
> 下記コマンドの `--project $(pwd)` は cwd を Serena の対象プロジェクトとして登録するため、
> 必ず SUT ディレクトリに `cd` した状態で実行してください。未取得の場合は先に `git clone` 等で SUT を準備します。

```bash
# uvx を使用してインストール
pip install uv  # uv がインストールされていない場合

# Serena の MCP サーバーを追加 (SUT ディレクトリで実行)
